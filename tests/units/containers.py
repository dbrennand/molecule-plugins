...
def test_container_driver_is_detected(registered_drivers):
    """Asserts that molecule recognizes the driver."""
    assert "containers" in registered_drivers


def test_container_driver_provides_schema(schema_path_for):
    """Asserts that the containers driver provides a JSON schema file."""
    schema_path = schema_path_for("containers")

    assert schema_path.is_file()
    assert schema_path.as_posix().endswith("containers/schema/driver.json")


def test_container_driver_schema_is_valid(schema_for):
    """Asserts that the containers driver schema is a valid JSON Schema."""
    schema = schema_for("containers")
...
def test_container_driver_schema_validation(schema_for, config, valid):
